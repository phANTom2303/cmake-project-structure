...
int main(void) {
  ...
  char url[256];
  snprintf(url, sizeof(url), "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=%s", api_key);

  // The JSON data to send in the POST request
  const char *post_data = "{\"contents\":[{\"parts\":[{\"text\":\"Explain how AI works in a few words\"}]}]}";
